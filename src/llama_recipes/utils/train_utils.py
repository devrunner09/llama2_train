...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            checkpoint_name = "finetuned_model_e{}".format(epoch)
            print("Checkpoint name: {}".format(checkpoint_name))
            
            model.save_pretrained(train_config.output_dir + checkpoint_name + "_after_epoch")   
        ...
        wandb.log({"loss_by_epoch": train_epoch_loss})
        if train_config.enable_fsdp:
        ...
        if train_config.run_validation:
            ...
            wandb.log({"eval_loss_by_epoch": eval_epoch_loss})
            ...
            if train_config.save_model and eval_epoch_loss < best_val_loss:
                ...
                if train_config.use_peft:
                    if train_config.enable_fsdp:
                        ...
                        print(f"we are about to save the PEFT modules")
                    model.save_pretrained(train_config.output_dir + checkpoint_name)  
                    if train_config.enable_fsdp:
                        if rank==0: 
